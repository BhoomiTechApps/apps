...
document.getElementById('btn-start-usb').addEventListener('click', async () => {
    ...
    // A fresh pick supersedes any pending "Reconnect to saved folder" offer.
    pendingReconnectHandle = null;
    const reconnectBtn = document.getElementById('btn-reconnect-usb');
 ...
 * re-select only if no handle was saved. If permission needs to be
 * re-confirmed, a "Reconnect" button is shown instead of immediately
 * falling back to the full folder picker — see the note on
 * requestPermission()'s user-activation requirement below.
 */
...
async function restoreUsbSource() {
    ...
    showSetupScreen();
    setSetupStatus('Checking saved USB folder access…', true);

    // queryPermission() is a read-only check and never needs a user
    // gesture, so it's always safe to call automatically here. The
    // browser remembering "granted" from earlier in this session is the
    // common case, and that's a silent, zero-click restore — exactly
    // what should happen on a plain reload.
    let currentPermission = 'prompt';
    try { currentPermission = await handle.queryPermission({ mode: 'read' }); }
    catch (err) { console.warn('Could not query folder permission:', err); }

    if (currentPermission === 'granted') {
        await finishUsbRestore(handle);
        return;
    }

    // Not currently granted. requestPermission() — the call that can
    // actually show the browser's "Allow access?" prompt — only works
    // when invoked with transient user activation (i.e. synchronously
    // inside a real click handler). Calling it automatically here, on
    // page load, cannot show that prompt in any browser that enforces
    // the spec correctly; it just silently fails every time, which
    // previously made a perfectly intact saved folder look like it
    // needed the full re-pick dialog on every single reload. Instead,
    // surface a lightweight, explicit "Reconnect" button — one tap
    // re-requests permission on this SAME saved handle (a small native
    // browser prompt), with no need to re-browse to the folder at all.
    pendingReconnectHandle = handle;
    const reconnectBtn = document.getElementById('btn-reconnect-usb');
    if (reconnectBtn) reconnectBtn.style.display = 'block';
    setSetupStatus('Tap "Reconnect to saved USB folder" below to continue without re-browsing.', true);
}

document.getElementById('btn-reconnect-usb').addEventListener('click', async () => {
    if (!pendingReconnectHandle) return;
    const handle = pendingReconnectHandle;
    setSetupStatus('Requesting access…', true);

    let granted = false;
    try { granted = (await ensureHandlePermission(handle)) === true; }
    catch (err) { console.warn('requestPermission failed:', err); }

    if (!granted) {
        setSetupStatus('Access wasn\'t granted. Tap Reconnect to try again, or use "Select folder…" to choose it from scratch.', false);
        return;
    }
...
async function finishUsbRestore(handle) {
    pendingReconnectHandle = null;
    const reconnectBtn = document.getElementById('btn-reconnect-usb');
    if (reconnectBtn) reconnectBtn.style.display = 'none';
