...
const DEFAULT_SNIPPETS = [
  ...
  const SHOWN_O = '\uE000'; // private-use placeholder, restored to 'o' in step 4
  ...
  // 2. Clean inherent trailing 'o' from single final word letters safely
  const trailingPattern = /(?:(?<=[aeiouy])|(?<![^aeiouy ]{2}))o(?=[.,!?;\u0964 \s\u00A0]|$)/gi;
  state.output = state.output.replace(trailingPattern, '');

  // 3. Keep the terminal 'o' on processed double consonants (like 'tt') at word boundaries
  const doubleConsonantPattern = /\b([^aeiouy\s\d])\1(?=[.,!?;\u0964 \s\u00A0]|$)/gi;
  state.output = state.output.replace(doubleConsonantPattern, '$&o');

  // 4. Restore explicit protected matras and the displayed 'o'
