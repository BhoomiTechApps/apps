...
async function handleFileSelect(input) {
  ...
  if (window.onFileLoaded) window.onFileLoaded(currentFile);
}
...
async function processImage(file) {
  ...
  await new Promise((resolve) => {
    ...
    img.onerror = () => {
      URL.revokeObjectURL(url);
      showToast("This image format cannot be opened. Try JPEG or PNG.");
      resolve();
    };
  });
}

// Recognition now runs through a reusable, fully offline worker (js/studio.js).
async function runTesseract() {
  return window.runRecognition();
}

async function runTesseractLegacy() {
  const canvas = document.getElementById("canvas");
...
async function startRecording() {
  ...
  try {
    ...
    // Firefox cannot connect a microphone to a context at a different rate
    // than the device; fall back to the device's own rate.
...
function exportWav() {
  ...
  const wavBuf = buildWavBuffer(pcm16, audioCtx ? audioCtx.sampleRate : SAMPLE_RATE, 1);
  const blob   = new Blob([wavBuf], { type: "audio/wav" });
  // Kept with the page (bundle / collection) instead of downloading at once.
  if (window.onRecordingReady) {
    window.onRecordingReady(blob);
  } else {
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    const base = currentFile ? currentFile.name.replace(/\.[^.]+$/, "") : "recording";
    a.href = url; a.download = `${base}_narration.wav`; a.click();
...
function showToast(msg, duration) {
  ...
  // duration 0 = stay until replaced; longer messages stay longer.
  if (duration !== 0) {
    t._timer = setTimeout(() => t.classList.remove("show"), duration || Math.max(2500, msg.length * 60));
  }
}
